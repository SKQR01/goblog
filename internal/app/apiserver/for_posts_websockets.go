...
func (c *Client) readPump(srv *server) {
	...
	for {
		...
		err := c.conn.ReadJSON(rangeMessage)
		if err != nil {
		...
		records, err := srv.store.Post().GetRecords(rangeMessage.PageNumber, rangeMessage.PaginationSize, -1)
		if err != nil {
			continue
		}
...
func (c *Client) writePump() {
	...
	for {
		...
		case message, ok := <-c.send:
			...
			decodedMessage := &postsMessage{}
			err := json.Unmarshal(message, decodedMessage)
			if err != nil {
				log.Println(err)
			}

			c.conn.WriteJSON(decodedMessage)
